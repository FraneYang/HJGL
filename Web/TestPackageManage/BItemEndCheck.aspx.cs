...
{
    public partial class BItemEndCheck : PPage
    {
        #region 定义项
        /// <summary>
        /// 管线
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack && this.CurrUser != null)
            {
                ...
                this.ButtonList = BLL.ButtonPowerService.GetButtonPowerList(roleId, BLL.Const.BItemEndCheckMenuId);
       ...
       /// <param name="e"></param>
        protected void btnSave_Click(object sender, ImageClickEventArgs e)
        {
            if (ButtonList.Contains(BLL.Const.BtnSave) || this.CurrUser.Account == BLL.Const.AdminId)
            {
                if (!String.IsNullOrEmpty(this.ISO_ID))
                {
                    ...
                    {
                        newBItemEndCheck.EIC_CheckDate = DateTime.Parse(this.txtEIC_CheckDate.Text.Trim());
                    }
                    newBItemEndCheck.EIC_DealMan = this.txtEIC_DealMan.Text.Trim();
                    if (!String.IsNullOrEmpty(this.txtEIC_DealDate.Text))
                    {
                        newBItemEndCheck.EIC_DealDate = DateTime.Parse(this.txtEIC_DealDate.Text.Trim());
                    }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void gvBItemEndCheck_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (!String.IsNullOrEmpty(e.CommandArgument.ToString()))
            {
                if (e.CommandName == "del")
                {
                    ...
                    {
                        BLL.BItemEndCheckService.DeleteTP_BItemEndCheckByID(e.CommandArgument.ToString());
                        this.gvBItemEndCheckBind();
                    }
                    else
                    {
                ...
                if (e.CommandName == "RemarkLink")
                {
                    if (ButtonList.Contains(BLL.Const.BtnModify) || this.CurrUser.Account == BLL.Const.AdminId)
                    {
                        ...
                        var item = BLL.BItemEndCheckService.GetTP_BItemEndCheckByID(e.CommandArgument.ToString());
        ...
        /// 绑定列表
        /// </summary>
        protected void gvBItemEndCheckBind()
        {
            this.ButtonIsEnabled(false);
            this.EIC_ID = null;
            this.TextIsEmpty();
            ...
            if (!string.IsNullOrEmpty(this.ISO_ID))
            {
                var bItemEndCheck = BLL.BItemEndCheckService.GetTP_BItemEndCheckByISO_ID(ISO_ID);
                this.gvBItemEndCheck.Visible = true;
                this.gvBItemEndCheck.DataSourceID = null;
                this.gvBItemEndCheck.DataSource = bItemEndCheck;
                this.gvBItemEndCheck.DataBind();
            }
            else
            {
                this.gvBItemEndCheck.Visible = true;
                this.gvBItemEndCheck.DataSourceID = null;
                this.gvBItemEndCheck.DataSource = null;
                this.gvBItemEndCheck.DataBind();
            }
        }
