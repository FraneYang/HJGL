...
{
    public partial class DataUpdate : PPage
    {
        /// <summary>
        /// 按钮权限列表
        /// </summary>
        ...
        protected void btnSave_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (errorInfos.Count <= 0)
            {
                ...
                //    }
                //}
                //foreach (var u in jointInfosUpdate)
                //{
                //    BLL.PW_JointInfoService.UpdateJointInfo(u);
                //}
        ...
        protected void imgBtnInsert_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (jointInfos.Count > 0 || jointInfosUpdate.Count > 0)
            {
                List<Model.PW_JointInfo> lists = new List<Model.PW_JointInfo>();
                foreach (var item in jointInfos)
                {
                    lists.Add(item);
                }
                foreach (var item in jointInfosUpdate)
                {
                    lists.Add(item);
                }
                this.div1.Visible = true;
                this.Table4.Visible = false;
                this.gvJointInfo.DataSource = lists;
