...
{
    public partial class TrustManageEdit : PPage
    {
        #region 定义项
        /// <summary>
        /// 委托主键
       ...
       /// <param name="e"></param>
        protected void btnSave_Click(object sender, ImageClickEventArgs e)
        {
            if (ButtonList.Contains(BLL.Const.BtnSave) || this.CurrUser.Account == BLL.Const.AdminId)
            {
                if (this.gvTrustItem.Rows.Count > 0)
                {
                    ...
                    {
                        if (updatetrust.CH_TrustCode != this.txtCH_TrustCode.Text.Trim())
                        {
                            if (BLL.TrustManageEditService.IsExistTrustCode(this.CurrUser.ProjectId, this.txtCH_TrustCode.Text.Trim()))
                            {
                                ScriptManager.RegisterStartupScript(this, typeof(string), "_alert", "alert('此委托单号已经存在！')", true);
                                return;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        {
                            var q = from x in Funs.DB.PW_JointInfo where x.JOT_ID == item.JOT_ID select x;
                            if (q.Count() > 0 && (!String.IsNullOrEmpty(q.First().JOT_TrustFlag) && q.First().JOT_TrustFlag != "00"))
                            {
                                ScriptManager.RegisterStartupScript(this, typeof(string), "_alert", "alert('焊口" + item.JOT_JointNo + "已委托,请重新选择！')", true);
                                return;
                            }
                        }
        ...
        protected void imgSearch_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {
                ClientScript.RegisterStartupScript(ClientScript.GetType(), "myscript", "<script type='text/javascript'>ShowSearch('" + this.drpCH_TrustUnit.SelectedValue + "'," + this.drpInstallationId.SelectedValue + ");</script>");
            }
        }
            ...
            if (this.tvControlItem.SelectedNode != null && this.tvControlItem.SelectedNode.Depth == 4)
            {
                ...
                if (!string.IsNullOrEmpty(CH_TrustID))
                {
                    ...
                    if (trust != null)
                    {
                        ...
                        if (trustItems.Count > 0)
                        {
                            string Jot_id = trustItems.First().JOT_ID;
                            var pd = from p in BLL.Funs.DB.PW_JointInfo where p.JOT_ID == Jot_id select p.PW_PointID;
                            if (pd.Count() > 0)
                            {
                                PW_PointID = pd.FirstOrDefault();
                                this.lbtnToPoint.Text = BLL.PointManageService.GetPointByPointID(PW_PointID).PW_PointNo;
                            }
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            if (ButtonList.Contains(BLL.Const.BtnDelete) || this.CurrUser.Account == BLL.Const.AdminId)
            {
                ...
                {
                    var trust = BLL.TrustManageEditService.GetCH_TrustByID(CH_TrustID);
                    var check = BLL.CheckManageService.GetCheckByTrustId(CH_TrustID);
                    if (trust != null && !trust.CH_AuditDate.HasValue && check == 0)
                    {
                        //var item = BLL.TrustManageEditService.GetView_CH_TrustItemByCH_TrustID(CH_TrustID);
                        if (jointInfo != null)
                        {
                            foreach (string j in jointInfo)
                            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void lbtnToPoint_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(this.PW_PointID))
            {
                ClientScript.RegisterStartupScript(ClientScript.GetType(), "myscript", "<script type='text/javascript'>ShowToPoint('" + PW_PointID + "');</script>");
            }
            else
            {
